#!/usr/bin/env python3
"""Strict PRE-only recovery for the locked 9201-race successor universe.
...
ROLE='RETROSPECTIVE_PRE_SUCCESSOR_UNTOUCHED_ONLY_UNPROVEN_V1'
...
def main():
    ...
    ap.add_argument('--start-position',type=int,default=5001)
    ap.add_argument('--end-position',type=int,default=5500)
    ...
    a=ap.parse_args()
    if not 5001<=a.start_position<=a.end_position<=9201: raise SystemExit('invalid successor position range')
    ...
    receipt={
      'record':'KEIRIN_SUCCESSOR9201_PRE_ONLY_BULK_RECOVERY_RECEIPT_v1',
      'status':'PASS' if not rejected else 'PASS_WITH_REJECTIONS',
      'locked_9201_sha256':EXPECTED_CANDIDATE_SHA,
