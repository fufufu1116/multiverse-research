...
This candidate compresses operator interaction, not the authoritative Step1 state
machine.  It retrieves the public immutable-by-hash delivery material, verifies
exact decoded action hashes, reconstructs the already-reviewed Step1 payload,
and runs INIT -> CHUNK 00..12 -> ASSEMBLE -> SOURCE in the current shell.
No OAuth, authenticated API call, production mutation, or Runtime activation is
contained here.  DRAFT / REVIEW ONLY / NOT LIVE AUTHORITY.
"""
...
STEP1_LEN=4687
STEP1_SHA='bbb4dfc09f669dcba4b8a223b641e9fa81b7ccebda3d72b216d97e3177184b74'
B64_LEN=6252
B64_SHA='f7c353761edf26a0ddeb25a129a7b152a16cf587bf5b620b6421863aa25418b2'
INIT_LEN=2314
INIT_SHA='36259d43cea843a2d7cbac981b133f576149cc71f11177dd6b6de544872d31ed'
TEMPLATE_LEN=4155
TEMPLATE_SHA='5e9c26723e9d04bb65abd4917fd02e25d4e42c2ffc1b9eb052202bf3e649cdee'
ASSEMBLE_LEN=4839
ASSEMBLE_SHA='f84b9f18a33c66eaf49475491be8baca63e03dc752ef31432cdec7c50950ed31'
SOURCE_LEN=7000
SOURCE_SHA='566669bb53f693bb380598fcf0e6d25b20b79987e5d8abc3869ffe3e3ae7b109'
...
def body(cid):
    req=urllib.request.Request(API.format(cid),headers={'Accept':'application/vnd.github+json','User-Agent':'multiverse-v18-review-candidate'})
    with urllib.request.urlopen(req,timeout=20) as r: return json.load(r)['body']
def pieces(text,prefix,n):
    out=[]
    for i in range(1,n+1):
        m=re.search(r'`%s%d`:\s*\n`([A-Za-z0-9+/=]+)`'%(prefix,i),text)
        if not m: raise SystemExit('missing manifest piece %s%d'%(prefix,i))
        out.append(m.group(1))
    return ''.join(out)
def decoded(b64,n,h):
    raw=base64.b64decode(b64,validate=True)
    if len(raw)!=n or sha(raw)!=h: raise SystemExit('authoritative action invariant mismatch')
    return raw.decode('utf-8')
def main():
    a,b=body(PART_A),body(PART_B)
    init=decoded(pieces(a,'I',3),INIT_LEN,INIT_SHA)
    template=decoded(pieces(a,'C',5),TEMPLATE_LEN,TEMPLATE_SHA)
    assemble=decoded(pieces(b,'A',6),ASSEMBLE_LEN,ASSEMBLE_SHA)
    source=decoded(pieces(b,'S',8),SOURCE_LEN,SOURCE_SHA)
    # Step1 payload is reconstructed from the reviewed public v2 gate exactly as the
    # predecessor recovery runner did, then frozen invariants are rechecked.
    gate_url='https://raw.githubusercontent.com/fufufu1116/multiverse-research/19a14cfd019cceab199571b5d03d4dd0ba5bcd22/governance/MULTIVERSE_R1_STAGE1_PHASE_C_PRODUCTION_EXECUTION_OWNER_GATE_CANDIDATE_20260824_v1.json'
    with urllib.request.urlopen(gate_url,timeout=20) as r: gate=json.load(r)
...
'''
    ...
    chunks=[sb64[x:x+512] for x in range(0,len(sb64),512)]
    ...
    for n,c in enumerate(chunks):
        ...
        act=template
        vals={'__CHUNK__':c,'__INDEX__':f'{n:02d}','__EXPECTED_LENGTH__':str(ln),'__EXPECTED_SHA256__':CHUNK_HASHES[n]}
        for k,v in vals.items():
            if act.count(k)!=1: raise SystemExit('template placeholder cardinality mismatch')
            act=act.replace(k,v)
        actions.append(act)
    actions += [assemble,source]
    # Emit one shell line. Each authoritative action runs in the same current shell;
    # `|| exit $?` prevents any later stage after failure. SOURCE remains last and
    # therefore preserves its reviewed parent-shell/source/EXIT-trap semantics.
    print('; '.join('eval '+repr(x)+' || exit $?' for x in actions),end='')
