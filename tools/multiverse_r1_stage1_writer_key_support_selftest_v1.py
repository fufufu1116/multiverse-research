...
def _syntax_and_static_boundary_test() -> None:
    ...
    assert psrc.index("channel.create_fence(main_before)") < psrc.index("random_bytes(WRITER_ID_NONCE_BYTES)")
    assert psrc.index("channel.create_fence(main_before)") < psrc.index("random_bytes(WRITER_KEY_ENTROPY_BYTES)")
...
def _fence_blocks_second_provisioner_before_secret_capability_test() -> None:
    fake = FakeChannel(fence_status=422, secret_status=201)
    counter = {"calls": 0}
    try:
        provisioner.apply_once(
            channel_factory=lambda: fake,
            random_bytes=_random_source(counter),
            session_marker_factory=lambda: "1" * 32,
...
def _secret_204_has_exactly_one_attempt_and_no_retry_test() -> None:
    ...
    try:
        provisioner.apply_once(
            channel_factory=lambda: fake,
            random_bytes=_random_source(counter),
            session_marker_factory=lambda: "2" * 32,
...
def _successful_candidate_path_outputs_nonsecret_commitment_only_test() -> None:
    ...
    result = provisioner.apply_once(
        channel_factory=lambda: fake,
        random_bytes=_random_source(counter),
        session_marker_factory=lambda: "3" * 32,
        encryptor=lambda _key, plain: "fake-ciphertext" if len(plain) == 44 else "bad",
    )
...
def main() -> int:
    ...
    _fence_blocks_second_provisioner_before_secret_capability_test()
