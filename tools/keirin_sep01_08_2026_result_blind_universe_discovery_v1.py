#!/usr/bin/env python3
"""Discover result-blind Sep 1-8, 2026 KDreams race universe from date indexes only."""
...
HOST='keirin.kdreams.jp';UA='Mozilla/5.0 (compatible; MultiverseKeirinResearch/6.1; UNIVERSE-DISCOVERY-ONLY)';MAX_BYTES=8*1024*1024
...
def discover_day(d,timeout):
    ...
    for a in soup.find_all('a',href=True):
        href=urljoin(f'https://{HOST}',a['href']);m=re.fullmatch(r'https://keirin\.kdreams\.jp/([^/]+)/racedetail/(\d+)/?',href)
        if m:
            slug,rid=m.groups();found[rid]=(rid,d.isoformat(),int(rid[:2]),'UNKNOWN',f'https://{HOST}/{slug}/racedetail/{rid}/','DISCOVERED')
...
def main():
    ...
    for d in days(date(2026,9,1),date(2026,9,8)):rows.extend(discover_day(d,a.timeout))
    rows=sorted({r[0]:r for r in rows}.values(),key=lambda r:(r[1],r[2],int(r[0][-4:])))
    b=canonical(rows);sha=hashlib.sha256(b).hexdigest();Path(a.out_csv).parent.mkdir(parents=True,exist_ok=True);Path(a.out_csv).write_bytes(b)
    rec={'record':'KEIRIN_SEP01_08_2026_RESULT_BLIND_UNIVERSE_DISCOVERY_v1','status':'DISCOVERY_ONLY_NO_DETAIL_PAGE_ACCESS',
      'date_start':'2026-09-01','date_end':'2026-09-08','candidate_races':len(rows),'canonical_csv_sha256':sha,
      'detail_pages_fetched':False,'result_fields_accessed':False,'payout_fields_accessed':False,'odds_fields_accessed':False,'model_fields_accessed':False,
    ...
    print('PASS_SEP_DISCOVERY',json.dumps(rec,sort_keys=True))
