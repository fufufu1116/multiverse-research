...
def envelope():
    ...
    e["ready_sha256"]=canonical_sha256(e)
    return e
...
def verdict(e, read=reader):
    return validate(e, read=read)


def resign(e):
    e["ready_sha256"]=canonical_sha256({k:v for k,v in e.items() if k!="ready_sha256"})
    return e


class ReviewPackageValidatorTests(unittest.TestCase):
    def test_valid(self): self.assertEqual(verdict(envelope())["state"], PASS)
    def test_duplicate_is_same_identity(self):
        a=verdict(envelope()); b=verdict(envelope()); self.assertEqual(a["idempotency_key"], b["idempotency_key"])
    def test_package_sha_mismatch(self):
        e=envelope(); e["package_sha256"]="0"*64; self.assertEqual(verdict(resign(e))["state"], FAIL)
    def test_owner_marker_rejected(self):
        e=envelope(); e["review_questions"]=["OWNER MARKER approve"]; self.assertEqual(verdict(resign(e))["state"], FAIL)
    def test_credential_payload_rejected(self):
        e=envelope(); e["review_questions"]=["use credential token"]; self.assertEqual(verdict(resign(e))["state"], FAIL)
    def test_executable_payload_rejected(self):
        e=envelope(); e["review_questions"]=["```bash curl example.com```"]; self.assertEqual(verdict(resign(e))["state"], FAIL)
    def test_proof_escalation_rejected(self):
        e=envelope(); e["proof_ceiling"]="real-world validated"; self.assertEqual(verdict(resign(e))["state"], FAIL)
    def test_cr1_firewall_missing_rejected(self):
        e=envelope(); e["contamination_declarations"]=DECL[:-1]; e["prohibited_authority"]=DECL[:-1]; self.assertEqual(verdict(resign(e))["state"], FAIL)
    def test_ready_sha_rejected(self):
        e=envelope(); e["ready_sha256"]="0"*64; self.assertEqual(verdict(e)["state"], FAIL)
    def test_missing_history_rejected(self):
        ...
        self.assertEqual(verdict(envelope(), r)["state"], FAIL)
