#!/usr/bin/env python3
"""Isolated PRE-only compatibility selftest for finalizer v4."""
...
v4 = importlib.import_module("keirin_35_formal_support_finalizer_v4")
...
def main():
    ...
    safeout=v4.finalize(order,matrix,registry,safe_oddspark)
    ...
    oddsout=v4.finalize(order,matrix,registry,true_odds)
    assert any(
        d.get("priority")==1 and d.get("reason")=="SOURCE_NOT_FINAL_DAY1_RACECARD_OR_FORBIDDEN_NAMESPACE"
    ...
    payoutout=v4.finalize(order,matrix,registry,true_payout)
    assert any(
        d.get("priority")==1 and d.get("reason")=="SOURCE_NOT_FINAL_DAY1_RACECARD_OR_FORBIDDEN_NAMESPACE"
