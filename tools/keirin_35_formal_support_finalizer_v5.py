#!/usr/bin/env python3
"""Relaxed-time PRE-only finalizer v5 for the locked 35-rider Keirin front.
...
This is a v5 candidate implementation. It keeps earlier versions immutable,
preserves source-namespace hardening, and makes the unambiguous support-gate
dimensions part of the emitted status instead of diagnostics-only. The
calendar-block dimension remains explicitly pending PR177 and therefore cannot
be represented as a complete support-gate PASS here.
"""
...
def _safe_source_role(row: dict) -> bool:
    """Accept only racecard roles while rejecting forbidden page namespaces.

    Important: do not scan the URL hostname. A legitimate trusted provider
    hostname such as oddspark.com contains the literal token "odds".
    """
...
def finalize(order: dict, matrix: dict, registry: dict, future_pre: dict) -> dict:
    ...
    max_venue_share = (max_venue / total) if total else None
    max_rider_share = (max_rider / total) if total else None
    venues_400 = venues_by_bucket.get("400", set())
    venues_333 = venues_by_bucket.get("333_OR_333_33", set())

    counts_reached = (
        new_pass == base.TARGET_NEW_PASS
        and cumulative_riders == base.TARGET_RIDERS
        and cumulative_rows == base.TARGET_ROWS
        and total == base.TARGET_ROWS
    )
    unambiguous_gate_pass = bool(
        counts_reached
        and max_venue_share is not None
        and max_venue_share <= 0.60
        and max_rider_share is not None
        and max_rider_share <= 0.10
        and len(venues_400) >= 2
        and len(venues_333) >= 2
    )

    if not counts_reached:
        status = "TARGET_NOT_REACHED_FAIL_CLOSED"
    elif not unambiguous_gate_pass:
        status = "COUNTS_REACHED_UNAMBIGUOUS_SUPPORT_GATE_FAIL_CLOSED"
    else:
        status = "COUNTS_30R_60ROWS_UNAMBIGUOUS_GATES_PASS_CALENDAR_BLOCK_PENDING_PR177"

    return {
        "record": "KEIRIN_35_FORMAL_SUPPORT_FINALIZATION_OUTPUT_v5",
        "status": status,
        ...
        "counts_reached": counts_reached,
        "unambiguous_support_gate_pass": unambiguous_gate_pass,
        "calendar_block_gate": "UNKNOWN_PENDING_ADOPTED_PR177_DEFINITION",
        "complete_support_gate_pass": False,
        "decisions": decisions,
        "receipts": receipts,
        "updated_witness_registry": updated_registry,
        "unambiguous_gate_diagnostics": {
            "identity_mapping_rate": 1.0 if new_pass else None,
            ...
            "minimum_distinct_venues_per_circumference": 2,
