...
def test_holdout_open_exactly_once_concurrent_restart(tmp_path):
    m=api(); p=tmp_path/'open.json'; audit=tmp_path/'audit.jsonl'; key=b'k'*32
    g1=m.AuditedHoldoutGate(p,key,audit); g2=m.AuditedHoldoutGate(p,key,audit)
    g1.initialize(b'synthetic-payload')
    first=g1.open_once(key)
    assert first==b'synthetic-payload'
    with pytest.raises(Exception): g2.open_once(key)
    st=m.AuditedHoldoutGate(p,key,audit).state()
    assert st['open_count']==1 and st['nonce']==2
