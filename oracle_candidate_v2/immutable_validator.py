...
MANIFEST = ROOT / "ZERO_AUDIT_ORACLE_V2_MANIFEST.json"
...
POISON={"conftest.py","pytest.py","sitecustomize.py","usercustomize.py"}

def verify_candidate_surface(stage: str):
    base=ROOT/"candidate/security_v3_3"
    for p in base.rglob("*"):
        if p.is_symlink(): raise SystemExit(f"FAIL_CLOSED: {stage}: candidate symlink")
        if p.is_file() and (p.name in POISON or p.suffix==".pth"):
            raise SystemExit(f"FAIL_CLOSED: {stage}: poisoning surface {p.name}")
