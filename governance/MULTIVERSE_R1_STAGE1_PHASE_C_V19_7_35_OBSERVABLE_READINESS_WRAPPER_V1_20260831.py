...
ROOT='/dev/shm/multiverse-r1-stage1-phase-c-v19-7-35-review'
TARGET=ROOT+'/governance/MULTIVERSE_R1_STAGE1_PHASE_C_V19_7_35_PRE_OAUTH_SEALED_DEPENDENCY_READINESS_V1_20260831.py'
TARGET_BLOB='f9ab4b4b1918f8a66ad917bd4c3779a5183f9725'
RESULT_DIR='/dev/shm/multiverse-r1-stage1-phase-c-v19-7-35-readiness-result'
FALLBACK_DIR='/dev/shm/multiverse-r1-stage1-phase-c-v19-7-35-readiness-receipt-failure'
...
def persist_at(dirpath,line):
 ...
 path=dirpath+'/'+RESULT_NAME; fd=os.open(path,os.O_WRONLY|os.O_CREAT|os.O_EXCL|getattr(os,'O_NOFOLLOW',0),0o600)
 try:
...
def receipt_failure(reason):
 line='PHASE_C_V19_7_35_RECEIPT_FAILURE:'+reason; print(line,flush=True)
 try: persist_at(FALLBACK_DIR,line)
 except Exception as e: print('PHASE_C_V19_7_35_FALLBACK_RECEIPT_FAILURE:'+type(e).__name__,flush=True)
 return 93
def finish(line,rc):
...
def read_target_once():
 ...
 return data
def main():
 if os.environ.get('CODESPACES')!='true' or not os.environ.get('CODESPACE_NAME'): return finish('PHASE_C_V19_7_35_DENIED:CODESPACES',92)
 if os.path.lexists(RESULT_DIR): return receipt_failure('RESULT_DIR_PREEXISTS')
 if os.path.lexists(FALLBACK_DIR): print('PHASE_C_V19_7_35_FALLBACK_RECEIPT_DIR_PREEXISTS',flush=True); return 93
 try: target=read_target_once()
 except Exception as e: return finish('PHASE_C_V19_7_35_DENIED:TARGET_'+type(e).__name__,92)
 env={'PATH':'/usr/local/bin:/usr/bin:/bin','CODESPACES':os.environ['CODESPACES'],'CODESPACE_NAME':os.environ['CODESPACE_NAME']}
 loader='import sys; d=sys.stdin.buffer.read(); exec(compile(d,"<v19.7.35-readiness-exact-same-memory>","exec"),{"__name__":"__main__"})'
 ...
 if cp.returncode:
  ...
  for line in out.splitlines():
   if line.startswith('PHASE_C_V19_7_35_READINESS_DENIED:'): reason=line
  return finish(reason or f'PHASE_C_V19_7_35_DENIED:UNKNOWN_RC_{cp.returncode}',92)
 if 'PHASE_C_V19_7_35_PRE_OAUTH_READINESS_PASS' not in out.splitlines(): return finish('PHASE_C_V19_7_35_DENIED:PASS_MARKER_MISSING',92)
 return finish('PHASE_C_V19_7_35_PRE_OAUTH_READINESS_PASS',0)
