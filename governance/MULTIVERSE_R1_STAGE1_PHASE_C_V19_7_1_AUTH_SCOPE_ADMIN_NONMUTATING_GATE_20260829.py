...
SUCCESS = "PHASE_C_V19_7_AUTH_SCOPE_ADMIN_NONMUTATING_PASS"
FAIL = "PHASE_C_V19_7_AUTH_SCOPE_ADMIN_NONMUTATING_STOP_DELETE_CODESPACE"
...
def main() -> None:
    ...
    if not (os.path.isfile(GH_BIN) and os.access(GH_BIN, os.X_OK)):
        stop("GH_BINARY")

    status, headers, user = api_get("/user")
    ...
    status, _, repo = api_get(f"/repos/{EXPECTED_REPO}")
