...
PY="/usr/local/python/current/bin/python"; HOST="files.pythonhosted.org"; ROOT=pathlib.Path("/dev/shm/multiverse-r1-stage1-phase-c-pydeps"); DL=ROOT/"wheels"
...
def stage(name,fn,*a):
 ...
 except BaseException as e: die(name+"_"+type(e).__name__)
...
def trusted_chain(path,allow_missing_leaf=False):
 root=pathlib.Path(os.path.realpath("/usr/local/python/current")); p=pathlib.Path(path); target=p.parent.resolve(strict=True) if allow_missing_leaf and not p.exists() else p.resolve(strict=True)
...
def trusted_stdlib_paths():
 ...
 trusted_chain(pyreal); mm=f"python{sys.version_info.major}.{sys.version_info.minor}"; compact=f"python{sys.version_info.major}{sys.version_info.minor}.zip"; std=os.path.join(trust,"lib",mm); dyn=os.path.join(std,"lib-dynload"); z=os.path.join(trust,"lib",compact)
...
def fetch(n,u,h):
 ...
 p=DL/n; conn=None; chunks=[]; dig=hashlib.sha256()
 try:
  conn=http.client.HTTPSConnection(HOST,443,timeout=60,context=ssl.create_default_context()); conn.request("GET",urllib.parse.urlunsplit(("","",x.path,x.query,"")),headers={"Host":HOST,"User-Agent":"multiverse-v19.7.34-readiness","Connection":"close"}); r=conn.getresponse()
  ...
  with open(p,"xb") as f:
   while True:
    b=r.read(1048576)
    if not b: break
    f.write(b); chunks.append(b); dig.update(b)
 except SystemExit: raise
 except BaseException as e: die("DOWNLOAD_"+type(e).__name__)
 finally:
  if conn is not None:
   try: conn.close()
   except Exception: pass
 data=b"".join(chunks)
 if dig.hexdigest()!=h or hashlib.sha256(data).hexdigest()!=h: die("ARTIFACT_SHA256")
 return p,data,h
def write_all(fd,data):
 v=memoryview(data); done=0
 while done<len(v):
  try: n=os.write(fd,v[done:])
  except BaseException as e: die("MEMFD_WRITE_"+type(e).__name__)
...
def sealed(name,data):
 ...
 try: fd=os.memfd_create(name,getattr(os,"MFD_CLOEXEC",0)|os.MFD_ALLOW_SEALING)
 except BaseException as e: die("MEMFD_CREATE_"+type(e).__name__)
 try:
  write_all(fd,data); stage("MEMFD_FSYNC",os.fsync,fd); expected=fcntl.F_SEAL_SEAL|fcntl.F_SEAL_SHRINK|fcntl.F_SEAL_GROW|fcntl.F_SEAL_WRITE; stage("MEMFD_ADD_SEALS",fcntl.fcntl,fd,fcntl.F_ADD_SEALS,expected)
  if stage("MEMFD_GET_SEALS",fcntl.fcntl,fd,fcntl.F_GET_SEALS)&expected!=expected: die("MEMFD_SEALS")
...
def wheel_parts(data,kind):
 try:
  with zipfile.ZipFile(io.BytesIO(data)) as z:
   ...
   if kind=="pynacl": c=[i for i in sos if pathlib.PurePosixPath(i.filename).name.startswith("_sodium")]
   else: c=[i for i in sos if pathlib.PurePosixPath(i.filename).name.startswith("_cffi_backend")]
   if len(c)!=1 or len(sos)!=1: die(kind.upper()+"_EXTENSION_SET")
   return z.read(c[0])
 except SystemExit: raise
 except BaseException as e: die("WHEEL_PARSE_"+type(e).__name__)
def spec(fullname,fd):
 ...
 except BaseException as e: die("EXTENSION_SPEC_"+type(e).__name__)
 if s is None: die("EXTENSION_SPEC_NONE")
 return s
def load_ext(fullname,fd):
 try: s=spec(fullname,fd); m=importlib.util.module_from_spec(s); sys.modules[fullname]=m; s.loader.exec_module(m); return m
 except SystemExit: raise
 except BaseException as e: die("LOAD_"+fullname.replace(".","_").upper()+"_"+type(e).__name__)
class SodiumFinder:
...
def probe(wheels,stdlib):
 ...
 try:
  ...
  z=[f"/proc/self/fd/{wf[n]}" for n in ("pynacl.whl","cffi.whl","pycparser.whl")]; sys.path[:]=z+stdlib; importlib.invalidate_caches(); load_ext("_cffi_backend",ef["_cffi_backend"]); finder=SodiumFinder(ef["nacl._sodium"]); sys.meta_path.insert(0,finder)
  try: nacl=importlib.import_module("nacl"); public=importlib.import_module("nacl.public")
  except BaseException as e: die("IMPORT_NACL_"+type(e).__name__)
  if finder in sys.meta_path: sys.meta_path.remove(finder)
  if getattr(nacl,"__version__",None)!="1.6.2": die("PYNACL_VERSION")
  try: k=public.PrivateKey.generate(); m=b"multiverse-v19.7.34-sealed-probe"; c=public.SealedBox(k.public_key).encrypt(m); out=public.SealedBox(k).decrypt(c)
  except BaseException as e: die("PYNACL_CRYPTO_"+type(e).__name__)
...
def main():
 ...
 stdlib=stage("TRUSTED_STDLIB",trusted_stdlib_paths)
 if len(pathlib.Path("/proc/swaps").read_text().splitlines())>1: die("ACTIVE_SWAP")
 if ROOT.exists(): die("PYDEPS_ROOT_PREEXISTS")
 stage("ROOT_MKDIR",ROOT.mkdir,0o700); stage("DL_MKDIR",DL.mkdir,0o700)
 if stage("PYDEPS_FS",fst,ROOT) not in {"tmpfs","ramfs"}: die("PYDEPS_NOT_MEMORY_FS")
 st=os.lstat(ROOT)
 if st.st_uid!=os.geteuid() or stat.S_IMODE(st.st_mode)!=0o700: die("PYDEPS_PERMISSIONS")
 a=dict(ART); a["cffi.whl"]=CFFI[sys.version_info[:2]]; fetched={n:fetch(n,*v) for n,v in a.items()}; wheels={n:t[1] for n,t in fetched.items()}; probe(wheels,stdlib)
 try:
  with open(ROOT/"MANIFEST.sha256","x",encoding="ascii") as f:
   for n in ("pynacl.whl","pycparser.whl","cffi.whl"): f.write(fetched[n][2]+"  wheels/"+n+"\n")
 except BaseException as e: die("MANIFEST_WRITE_"+type(e).__name__)
...
if __name__=="__main__":
 ...
 except BaseException as e: die("UNCAUGHT_"+type(e).__name__)
