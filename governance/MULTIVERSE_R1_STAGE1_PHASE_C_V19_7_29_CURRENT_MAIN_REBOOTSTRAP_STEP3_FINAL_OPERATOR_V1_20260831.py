...
def must_git(args: list[str]) -> str:
    cp = run_git(args)
    if cp.returncode != 0:
...
def assert_tmpfs_dir(path: str, mode: int = 0o700) -> os.stat_result:
    st = os.lstat(path)
...
def assert_repo(root: str, expected_head: str, expected_tree: str) -> os.stat_result:
    root_st = assert_tmpfs_dir(root)
    dotgit = os.path.join(root, ".git")
    git_st = os.lstat(dotgit)
    ...
    if must_git(["-C", root, "rev-parse", "--verify", "HEAD^{tree}"]) != expected_tree:
...
def rename_exchange(path_a: str, path_b: str) -> None:
...
def rebootstrap_current_main() -> None:
    stale_before = assert_repo(EXEC_ROOT, STALE_MAIN, STALE_TREE)
    ...
    new_before = assert_repo(NEW_ROOT, CANONICAL_MAIN, CANONICAL_TREE)

    stale_now = assert_repo(EXEC_ROOT, STALE_MAIN, STALE_TREE)
    ...
    rename_exchange(EXEC_ROOT, NEW_ROOT)

    current_after = os.lstat(EXEC_ROOT)
    old_after = os.lstat(NEW_ROOT)
    if (current_after.st_dev, current_after.st_ino) != (new_before.st_dev, new_before.st_ino):
        raise RuntimeError("CURRENT_ROOT_EXCHANGE_IDENTITY_MISMATCH")
    if (old_after.st_dev, old_after.st_ino) != (stale_before.st_dev, stale_before.st_ino):
        raise RuntimeError("STALE_ROOT_EXCHANGE_IDENTITY_MISMATCH")
    assert_repo(EXEC_ROOT, CANONICAL_MAIN, CANONICAL_TREE)
    assert_repo(NEW_ROOT, STALE_MAIN, STALE_TREE)

    os.rename(NEW_ROOT, STALE_ROOT)
    stale_final = os.lstat(STALE_ROOT)
    if (stale_final.st_dev, stale_final.st_ino) != (stale_before.st_dev, stale_before.st_ino):
        raise RuntimeError("STALE_QUARANTINE_IDENTITY_MISMATCH")
    assert_repo(EXEC_ROOT, CANONICAL_MAIN, CANONICAL_TREE)
    assert_repo(STALE_ROOT, STALE_MAIN, STALE_TREE)
