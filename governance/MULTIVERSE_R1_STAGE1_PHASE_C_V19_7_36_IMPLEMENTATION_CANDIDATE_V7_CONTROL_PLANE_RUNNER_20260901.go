...
func openC(p string) (*os.File, error) {
	...
	if u != 0 || st.Mode().Perm()&0022 != 0 || !st.Mode().IsRegular() { f.Close(); return nil, errors.New("class-c") }
	return f, nil
}
...
func runfd(p string, args []string, ev []string, timeout time.Duration) ([]byte, []byte, error) {
	f, e := openC(p); if e != nil { return nil,nil,e }; defer f.Close()
	c := exec.Command("/proc/self/fd/3", args...); c.ExtraFiles=[]*os.File{f}; c.Env=ev
	var o,r bytes.Buffer; c.Stdout=&o; c.Stderr=&r
	if timeout>0 { done:=make(chan error,1); go func(){done<-c.Run()}(); select { case e=<-done: case <-time.After(timeout): if c.Process!=nil {_=c.Process.Kill()}; e=errors.New("timeout") } } else { e=c.Run() }
...
func buildSelftest() {
	...
	if e!=nil || !strings.Contains(string(o),"refs/heads/main") { die("GIT_LS_REMOTE") }
	if _,_,e=runfd(ghPath,[]string{"version"},env(),5*time.Second); e!=nil { die("GH_VERSION") }
	_,_,e=runfd(browserPath,nil,env(),2*time.Second); if x,ok:=e.(*exec.ExitError); !ok || x.ExitCode()!=1 { die("BROWSER_FALSE") }
	fmt.Println("PHASE_C_V19_7_36_V7_CONTROL_BUILD_SELFTEST_PASS")
}
func postOAuth() {
	d:=os.Getenv("GH_CONFIG_DIR"); if d=="" || !strings.HasPrefix(d,"/dev/shm/") { die("GH_CONFIG_DIR") }
	...
	a:=os.Getenv("MULTIVERSE_V36_V7_CONTROL_ACTION"); args,ok:=allowed[a]; if !ok { die("ACTION") }
	o,r,e:=runfd(ghPath,args,ev,15*time.Second); if e!=nil { _,_=io.Copy(os.Stderr,bytes.NewReader(r)); die("GH_ACTION") }; _,_=os.Stdout.Write(o)
}
func main(){ if len(os.Args)!=2 {die("ARGV")}; switch os.Args[1] {case "build-selftest":buildSelftest(); case "post-oauth":postOAuth(); default:die("ACTION")} }
