#!/usr/bin/env python3
"""V19.7.36 v6 runtime verifier. REVIEW-ONLY / NO LIVE AUTHORITY."""
import hashlib,json,os,stat,sys
RC=92; AFD='MULTIVERSE_V36_V6_ATTEST_FD'; RFD='MULTIVERSE_V36_V6_RUNTIME_FD'; MFD='MULTIVERSE_V36_V6_MANIFEST_FD'
...
def deny(x): print('PHASE_C_V19_7_36_V6_DENIED:'+x,flush=True);raise SystemExit(RC)
...
def manifest():
 ...
 if m.get('version')!='V19.7.36-v6':deny('MANIFEST_VERSION')
...
def att():
 ...
 if a.get('version')!='V19.7.36-v6' or a.get('source')!='ROOT_IMAGE_ANCHOR_PRODUCER_V6':deny('ATTEST_SCHEMA')
 q=a.get('matrix',{})
...
def dependency_probe():
 ...
 try:
  ...
  sk=PrivateKey.generate();msg=b'v36-v6';ct=SealedBox(sk.public_key).encrypt(msg)
  if SealedBox(sk).decrypt(ct)!=msg:deny('PYNACL_ROUNDTRIP')
 except SystemExit:raise
 except BaseException as e:deny('PYNACL_'+type(e).__name__.upper())
def step3_binding(idx):
 bp='/opt/multiverse/v36/step3-binding.json'; sp='/opt/multiverse/v36/step3.py'
 for p in (bp,sp):
  e=idx.get(p)
  if e is None:deny('STEP3_UNMANIFESTED')
  ident(p,e)
 try:
  with open(bp,'rb',buffering=0) as f:b=f.read()
  q=json.loads(b.decode())
 except Exception:deny('STEP3_BINDING_JSON')
 s=q.get('step3',{})
 if q.get('version')!='V19.7.36-v6' or s.get('path')!=sp or s.get('mode')!='NONMUTATING' or s.get('mutations')!=0:deny('STEP3_BINDING_SCHEMA')
 e=idx[sp]
 if s.get('size')!=e.get('size') or s.get('sha256')!=e.get('sha256') or s.get('git_blob')!='1e6584749d99bc15d9e7147ecda2523a821dbd72':deny('STEP3_BINDING_IDENTITY')
def main():
 env_gate();m,idx=manifest();a=att();dependency_probe();used_paths(idx);step3_binding(idx)
 for i in range(1,17):
  d=a['matrix'][str(i)];print(f"PHASE_C_V19_7_36_V6_MATRIX_{i:02d}:{d['state']}:{d['evidence']}",flush=True)
 print('PHASE_C_V19_7_36_V6_PLATFORM_ANCHOR_PASS',flush=True);print('OAUTH_STARTED=false',flush=True);print('RUNTIME_ACTIVATION_PERFORMED=false',flush=True)
