...
const (
 ...
 expectedRuntimeSize = 4610
 expectedRuntimeSHA = "97ce2eb7a17c8e23ffe910397046cbefe7ffe2ddd26a5cf6b7e73fce716ab4dd"
...
func strongReceipt(payload []byte) error {
 ...
 var sfs syscall.Statfs_t;if e=syscall.Fstatfs(int(d.Fd()),&sfs);e!=nil||uint64(sfs.Type)!=tmpfsMagic{return errors.New("receipt root not tmpfs")}
 fd,e:=syscall.Openat(int(d.Fd()),"PRE_PYTHON.json",syscall.O_RDWR|syscall.O_CREAT|syscall.O_EXCL|syscall.O_NOFOLLOW|syscall.O_CLOEXEC,0400);if e!=nil{return e};f:=os.NewFile(uintptr(fd),"receipt");defer f.Close()
...
func shmOK()bool{var s syscall.Statfs_t;if syscall.Statfs("/dev/shm",&s)!=nil||uint64(s.Type)!=tmpfsMagic{return false};fi,e:=os.Stat("/dev/shm");return e==nil&&fi.IsDir()}
...
func matrix(codespaces,name string,py *os.File) (map[string]MatrixEvidence,error){
 ...
 if !shmOK(){return nil,errors.New("shm")};add(4,"/dev/shm tmpfs mechanically verified")
 if fi,e:=os.Stat("/proc/self/fd");e!=nil||!fi.IsDir(){return nil,errors.New("procfd")};add(5,"kernel /proc/self/fd available")
 add(6,"shell ENTRY removed; Python and runtime actual-use are inherited opened class-C objects")
 ...
 add(13,"review-freeze has zero Step3/apply authority; exact successor binding remains fail-closed")
 add(14,"gh credential proof POST_OAUTH_ONLY; executable/config/CA substrate pre-bound")
 add(15,"root producer openat O_EXCL/O_NOFOLLOW tmpfs receipt, verified/fynced/readback before Python")
 add(16,"receipt root required absent; platform image root producer is outside session UID authority")
 return m,nil
}
func runChain(codespaces,name string)error{
 ...
 b,e:=json.Marshal(att);if e!=nil{return e};go func(){defer aw.Close();off:=0;for off<len(b){n,er:=aw.Write(b[off:]);if er!=nil{return};if n<=0{return};off+=n}}()
