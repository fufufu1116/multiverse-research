...
func deny(code string) {
    fmt.Fprintln(os.Stderr, "PHASE_C_V19_7_36_V7R15_PROTECTED_GUARD_DENIED:"+code)
    os.Exit(92)
}
...
func verifyAuthorityFD(fd int, name, generation string) (authoritySnapshot,string,error) {
    ...
    f := os.NewFile(uintptr(fd),"v7r15-authority")
...
func irreversibleDropToUser(uid uint32) error {
    ...
    r,e,s,err := getResUIDs()
    if err != nil { return err }
    if r != uid || e != uid || s != uid { return fmt.Errorf("drop-resuid") }
    if err = setFSUIDAndVerify(uid); err != nil { return err }
    if err = setNondumpable(); err != nil { return err }
    return verifyNoTracer()
}
...
func selftest() {
    ...
    fmt.Println("PHASE_C_V19_7_36_V7R15_PROTECTED_GUARD_SELFTEST_PASS")
    fmt.Println("PROTECTED_GUARD_STARTUP_CREDENTIAL_MISMATCH_REQUIRED=true")
    fmt.Println("SEALED_AUTHORITY_FD_REQUIRED=true")
    fmt.Println("IRREVERSIBLE_DROP_BEFORE_HELPER_EXEC=true")
...
func main() {
    ...
    os.Clearenv()
    verifyHelper()
    if _,_,err = verifyAuthorityFD(fd,name,generation); err != nil { deny("SEALED_AUTHORITY_REVERIFY") }
    if err = irreversibleDropToUser(uid); err != nil { deny("IRREVERSIBLE_USER_DROP") }
    if _,_,err = verifyAuthorityFD(fd,name,generation); err != nil { deny("POST_DROP_AUTHORITY_REVERIFY") }
    if err = syscall.Close(fd); err != nil { deny("AUTHORITY_CLOSE") }
    env := []string{"CODESPACES=true","CODESPACE_NAME="+name}
    argv := []string{helperPath}
    fmt.Printf("PHASE_C_V19_7_36_V7R15_PROTECTED_GUARD_PASS codespace=%s generation=%s authority_sha256=%s rate=%d/%d authorization=SEALED_BEFORE_IRREVERSIBLE_DROP helper_sha256=%s runtime=OFF\n", name,generation,authoritySHA,snap.Before,snap.After,expectedHelperSHA256)
    if err := syscall.Exec(helperPath,argv,env); err != nil { deny("HELPER_EXEC") }
}
