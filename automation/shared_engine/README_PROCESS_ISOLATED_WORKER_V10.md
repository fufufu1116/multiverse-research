...
CI history for this two-finding remediation preserves push run `33837772666` / job `100913694275` as FAILURE: Fresh-main/exact binding passed, then the unchanged inherited v9 suite hit two sub-second lease-expiry timing failures (`test_restart_after_lab_fix_required_uses_durable_generation` and `test_crash_after_durable_provider_receipt_reuses_same_operation`) before the v10 gate could run. This evidence is retained as a runner-sensitive inherited timing failure; no v9 source/test semantics are modified or weakened in response.
