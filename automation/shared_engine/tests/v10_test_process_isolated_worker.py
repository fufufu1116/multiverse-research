...
class ProcessIsolatedWorkerV10Tests(unittest.TestCase):
    ...
    def raw(self, payload: bytes):
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as s:
            s.settimeout(2); s.connect(self.socket); s.sendall(payload); return s.recv(4096)

    def test_distinct_process_and_client_has_no_engine_globals(self):
        ping = self.client.ping(); self.assertNotEqual(ping['broker_pid'], os.getpid())
    ...
    def test_preexisting_core_and_keirin_tasks_use_same_client_broker_path(self):
        ...
        self.assertEqual(db.get_task(core)['state'], 'DONE'); self.assertEqual(db.get_task(keirin)['state'], 'DONE')

    def test_no_task_creation_opcode_and_bounds(self):
        self.assertEqual(client_module.ALLOWED_OPS, frozenset({'PING','STEP','STOP'}))
        with self.assertRaisesRegex(ValueError, 'V10_RUN_CYCLE_BOUND'): self.client.run(max_cycles=1001)
        with self.assertRaisesRegex(ValueError, 'V10_UNIX_SOCKET_PATH_BOUND'): ClientConfig('/' + 'x'*101).validate()
