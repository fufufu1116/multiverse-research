...
def main():
    ...
    require(m['candidate_only'] is True, 'V10_CANDIDATE_ONLY_REQUIRED')
    require(m['branch'] == EXPECTED_BRANCH, 'V10_BRANCH_BINDING')
    require(m['predecessor_head'] == EXPECTED_PREDECESSOR, 'V10_PREDECESSOR_BINDING')
    require(m['base_pr91_head'] == EXPECTED_PR91, 'V10_PR91_BINDING')
    require(m['stacked_v7_head'] == EXPECTED_V7, 'V10_V7_BINDING')
    require(m['canonical_main'] == EXPECTED_MAIN, 'V10_MAIN_BINDING')
    require(all(v is False for v in m['authority'].values()), 'V10_AUTHORITY_MUST_BE_FALSE')
    a = m['architecture']
    require(a['client_and_engine_distinct_os_processes'] is True, 'V10_PROCESS_ISOLATION_REQUIRED')
    require(a['ipc_ops'] == ['PING','STEP','STOP'], 'V10_EXACT_IPC_OPS')
