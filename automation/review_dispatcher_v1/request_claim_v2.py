...
CLAIM_SCHEMA = "MULTIVERSE_REVIEW_REQUEST_CLAIM_v2"
CLAIM_REF_PREFIX = "refs/tags/multiverse-review-claims/v2"
...
def claim_ref(value: dict[str, Any]) -> str:
    return f"{CLAIM_REF_PREFIX}/{envelope_sha256(value)}"


def build_claim(*, envelope: dict[str, Any], request_id: str, claimant: str, nonce: str, predecessor_sha256: str | None) -> dict[str, Any]:
...
def validate_claim(claim: dict[str, Any]) -> dict[str, Any]:
    ...
    require(set(claim) == {"schema", "envelope", "envelope_sha256", "claim_ref", "request_id", "claimant", "nonce", "predecessor_sha256", "claim_blob_sha1"}, "CLAIM_KEYS")
...
def acquire_claim(*, claim: dict[str, Any], create_blob: Callable[[bytes], str], create_ref: Callable[[str, str], None]) -> dict[str, Any]:
    ...
    return {"claim_ref": claim["claim_ref"], "claim_blob_sha1": blob_sha, "request_id": claim["request_id"]}


def verify_claim_binding(*, claim: dict[str, Any], observed_ref_sha: str, request_id: str, predecessor_sha256: str | None) -> None:
    validate_claim(claim)
