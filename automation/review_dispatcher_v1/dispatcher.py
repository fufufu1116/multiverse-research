...
from automation.review_dispatcher_v1.request_claim_v5 import verify_request_claim
...
def discover_pr(repo: str, head: str, fetch=github_get) -> dict[str, Any]:
    ...
    full_pr_raw = fetch(f"https://api.github.com/repos/{repo}/pulls/{pr_number}")
    return github_full_pr_binding(full_pr_raw, expected_number=pr_number, expected_head=head)


def _discover_tree_sha(repo: str, head: str, fetch=github_get) -> str:
    return github_commit_tree_sha(fetch(f"https://api.github.com/repos/{repo}/commits/{head}"))


def _discover_main_sha(repo: str, fetch=github_get) -> str:
    return github_branch_commit_sha(fetch(f"https://api.github.com/repos/{repo}/branches/main"))


def discover_request(*, repo: str, lane: str, head: str, fetch=github_get) -> dict[str, Any]:
    ...
    comments = fetch_all_pages(fetch, f"https://api.github.com/repos/{repo}/issues/{pr_number}/comments")
    require(all(isinstance(item, dict) for item in comments), "COMMENTS_ITEM_OBJECT")
    ...
    marker = result_marker(request["request_id"], head, request_comment, request_sha256)
    duplicate_ids: list[int] = []
    for item in comments:
        if marker in (item.get("body") or "") and lane_result_comment_trusted(item, lane):
            duplicate_ids.append(github_comment_id(item, "RESULT_COMMENT_ID"))
    require(
        not duplicate_ids,
        "CURRENT_REQUEST_RESULT_ALREADY_EXISTS:" + ",".join(str(i) for i in duplicate_ids),
    )
    ...
    return {
        ...
        "request_claim_commit_sha": claim["claim_commit_sha"] if claim else "",
        "request": request,
...
def main() -> int:
    ...
    parser.add_argument("--output", default="review_job.json")
    ...
    job = discover_request(repo=args.repo, lane=args.lane, head=head)
        ...
        json.dumps(
            {
                ...
                "request_claim_commit_sha": job["request_claim_commit_sha"],
                "lane": job["lane"],
                "pr": job["pr"],
                "head": job["head"],
                "tree": job["tree"],
