...
def verify(stage):
    cp=subprocess.run([sys.executable,"oracle_candidate_v2/immutable_validator.py","--verify-manifest",stage],cwd=ROOT,text=True,capture_output=True)
...
def run_tests(label: str):
    ...
    cp=subprocess.run([sys.executable,"oracle_candidate_v2/immutable_validator.py","--evidence-dir",str(d.relative_to(ROOT)),"--report",str(report.relative_to(ROOT))],cwd=ROOT,text=True,capture_output=True)
...
def call_gemini(client, attempt, failure):
    tests=(ROOT/"oracle_candidate_v2/test_security_oracle.py").read_text(encoding="utf-8")
    spec=(ROOT/"oracle_candidate_v2/ORACLE_SPEC.json").read_text(encoding="utf-8")
