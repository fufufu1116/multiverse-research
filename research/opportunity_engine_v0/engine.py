...
def simulate_unit_economics(
    *,
    ...
    if min(
        profit_per_conversion_yen,
        ad_revenue_per_1000_visitors_yen,
        variable_cost_per_visitor_yen,
        fixed_cost_yen,
    ) < 0:
    ...
    return {
        ...
        "conversion_profit_yen": conversion_profit,
        "advertising_profit_yen": advertising_profit,
        "variable_cost_yen": variable_cost,
        "fixed_cost_yen": fixed_cost_yen,
...
def evaluate(candidate: OpportunityCandidate) -> Evaluation:
    ...
    if not candidate.evidence_verified:
        hard_failures.append("UNVERIFIED_EVIDENCE")
    if not candidate.competitor_research.complete:
        hard_failures.append("COMPETITOR_RESEARCH_INCOMPLETE")
    if candidate.deceptive_tactics_required:
        hard_failures.append("DECEPTIVE_TACTICS_REQUIRED")
    if candidate.unverified_personal_claims_required:
    ...
    if candidate.demand_life_days is not None:
        if candidate.build_days > candidate.demand_life_days * 0.40:
            hard_failures.append("BUILD_TOO_SLOW_FOR_DEMAND_LIFE")
        if not candidate.exit_trigger:
            hard_failures.append("MISSING_EXIT_TRIGGER")
    if len(candidate.future_steps) < 3:
        hard_failures.append("FUTURE_HORIZON_TOO_SHALLOW")

    if hard_failures:
        return Evaluation(
            decision=Decision.REJECT,
            score=0,
            revenue_route=choose_revenue_route(candidate),
    ...
    positive = sum(
        (
            _weighted(candidate.buyer_clarity, 9),
            _weighted(candidate.attention, 8),
            _weighted(candidate.purchase_intent, 10),
            _weighted(candidate.why_now, 8),
            _weighted(candidate.why_not_before, 5),
            _weighted(candidate.proprietary_edge, 12),
            _weighted(candidate.action_completion, 8),
            _weighted(candidate.reusable_asset, 10),
    ...
    if candidate.expected_profit_low_yen > candidate.initial_cost_yen:
        ...
        positive += 5
        reasons.append("BASE_CASE_PAYS_BACK_INITIAL_COST")
    else:
        reasons.append("PAYBACK_NOT_PROVEN")

    penalties = sum(
        (
            _weighted(candidate.competitor_pressure, 8),
            _weighted(candidate.incumbent_crush_risk, 10),
            _weighted(candidate.ai_substitutability, 8),
            _weighted(candidate.legal_risk, 10),
            _weighted(candidate.human_burden, 8),
        )
    )
    ...
    if candidate.demand_life_days is not None:
        ...
        if build_share <= 0.10:
            positive += 5
            reasons.append("FAST_ENOUGH_FOR_SHORT_LIVED_DEMAND")
        elif build_share > 0.25:
            penalties += 5
            reasons.append("DEMAND_LIFE_MARGIN_IS_THIN")
