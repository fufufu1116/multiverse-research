#!/usr/bin/env python3
"""
Shadow250-v2 race-level KEIRIN identity + supplement batch v2 candidate.
Candidate-only, NOT ACTIVE.

v2 preserves the v1 shared provider limiter / one-search-per-(pref,term) / one-profile-fetch-per-rider architecture,
but binds supplement parsing to the live-proven section/ownership-hardened racerprofile parser v4.
"""
...
SUPPLEMENT_PARSER_PATH=HERE/'keirinjp_racerprofile_parser_v4.py'
EXPECTED_LOCATOR_GIT_BLOB='41517a604bb0c23200cb4d0878d8459f9379d63f'
EXPECTED_SUPPLEMENT_PARSER_GIT_BLOB='d8ce951abd1f008f872bc093d6bb12a50d62ca16'
...
def _elapsed_guard(started,source_window_started):
    batch=time.monotonic()-started;window=time.monotonic()-float(source_window_started)
...
def _validate_entrants(entrants,loc):
    ...
    for e in entrants:
        ...
        if car<1 or car>7 or car in cars or not name or key in keys:raise FailClosed('FAIL_CLOSED entrant identity/cardinality')
...
def resolve_race(entrants,source_window_started_monotonic):
    ...
    loc=_load('locator_v2_batch2',LOCATOR_PATH);sup=_load('racerprofile_parser_v4_batch2',SUPPLEMENT_PARSER_PATH)
    ...
    es=_validate_entrants(entrants,loc);started=time.monotonic();s=requests.Session();lim=loc.RuntimeLimiter()
    landing=loc._request(s,lim,'get',loc.SEARCH_URL);_elapsed_guard(started,source_window_started_monotonic)
    groups={}
    ...
    for (pref,term),members in sorted(groups.items(),key=lambda kv:(kv[0][0],kv[0][1])):
        action,payload=loc._verify_search_landing(landing.content,pref,term);_wait();result=loc._request(s,lim,'get',action,params=payload);_elapsed_guard(started,source_window_started_monotonic)
        ...
        search_receipts.append({'prefecture':pref,'term':term,'member_cars':[e['car_no'] for e in members],'candidate_count':len(candidates),'content_sha256':hashlib.sha256(result.content).hexdigest(),'identity_fields_used':['name','snum']})
    profiles=[]
    for e in es:
        snum=resolved[e['car_no']];_wait();url=f"https://{loc.ALLOWED_HOST}{loc.PROFILE_PATH}?"+urlencode({'snum':snum});r=loc._request(s,lim,'get',url);batch_elapsed,window_elapsed=_elapsed_guard(started,source_window_started_monotonic)
        raw_hash=hashlib.sha256(r.content).hexdigest();capture=datetime.now(timezone.utc).isoformat();ident=loc.parse_identity_profile(r.content,url);loc.verify_identity(ident,e['rider_name'],e['prefecture'],e['term'],snum)
    ...
    return {'status':'PASS_RACE_BATCH_IDENTITY_AND_SUPPLEMENT_V2','rider_count':len(profiles),'group_count':len(groups),'request_count':1+len(groups)+len(profiles),'batch_elapsed_seconds':round(batch_elapsed,3),'source_window_elapsed_seconds':round(window_elapsed,3),'max_batch_elapsed_seconds':MAX_BATCH_ELAPSED_SECONDS,'max_source_window_seconds':MAX_SOURCE_WINDOW_SECONDS,'search_result_identity_fields_used':['name','snum'],'predictive_fields_from_search_result':[],'predictive_fields_from_racerprofile':['style','win_rate','trio_rate'],'raw_html_persisted':False,'search_receipts':search_receipts,'profiles':profiles}
def synthetic_tests():
    loc=_load('loc_synth_batch2',LOCATOR_PATH);good=[{'car_no':i,'rider_name':f'選手{i}','prefecture':'岡山','term':127} for i in range(1,8)];assert len(_validate_entrants(good,loc))==7
    try:_validate_entrants(good[:-1],loc);raise AssertionError('six riders accepted')
    except FailClosed:pass
    return {'status':'PASS','network_used':False,'race_batch_required':True,'supplement_parser_blob':EXPECTED_SUPPLEMENT_PARSER_GIT_BLOB}
