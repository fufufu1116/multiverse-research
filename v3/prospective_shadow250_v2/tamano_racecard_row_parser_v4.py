#!/usr/bin/env python3
"""
Shadow250-v2 Tamano deterministic PRE row parser v4.

Candidate-only, NOT ACTIVE.

v4 keeps v3 transport-provenance race-date binding and all row extraction
semantics, but fixes the field-size invariant using a dedicated, empirically
mapped PRE-only marker band. Live PRE-only mapping proved all 12 `(7車立)`
markers sit directly below their corresponding seven entrant rows. The parser
therefore checks only x=ox+60..ox+150 and y=ymax..ymax+30 for exactly one
`(7車立)` marker; it does not widen entrant-field extraction.
"""
...
def _field_size(words,ox,ymax):
    marker=[w for w in words if ox+60<=w[0]<=ox+150 and ymax<=w[1]<=ymax+30]
    text=norm("".join(w[4] for w in sorted(marker,key=lambda z:(z[1],z[0]))))
...
def parse_tamano_pdf(path,race_date,source_url,expected_raw_sha256):
    ...
    for pi,ox,rno,ymin,ymax in RACE_SPECS:
        ...
        size=_field_size(words,ox,ymax); program=_program_label(block)
        ...
        for car,wcar in cars:
            ...
            race_rows.append(dict(race_id=rid,race_date=race_date,venue="玉野",venue_code="61",race_no=rno,car_no=car,
                ...
                **{"class":cls,"score":score,"S":S,"B":B,"quinella_rate":qr,"withdrawn":False,
                   ...
                   "field_size_marker_basis":"DEDICATED_PRE_MARKER_BAND_X60_150_YMAX_0_30",
...
def synthetic_tests():
    ...
    # Dedicated field-size band synthetic.
    words=[(70,10,130,20,"(7車立)",0,0,0)]
    assert _field_size(words,0,10)==7
    try: _field_size([],0,10); raise AssertionError("missing marker not rejected")
    except FailClosed: pass
