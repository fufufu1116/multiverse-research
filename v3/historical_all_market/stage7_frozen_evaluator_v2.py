#!/usr/bin/env python3
"""Multiverse Hybrid v3.0 — frozen Stage-7 chronological economic evaluator v2.

Memory-hardened implementation of the audited Stage-7 protocol.
- Streams the exact Stage-2 4000-row catalog in immutable race/model order.
- Segment A: all 784 configs.
- Freeze A_TOP10, then Segment B: only A_TOP10.
- Atomically freeze exactly one FINAL_DEV2000_CONFIGURATION before C settlement is read.
- Segment C: score only the frozen config exactly once; atomic 500-race ledger.
- Whole-race bootstrap: 10,000 reps, seed 20260819, deterministic indices,
  bounded multi-thread pool, persistent batch logging.
- No model refit, no post-outcome tuning, no HOLDOUT access.
"""
...
def _validate_stage2_row(r: dict[str, Any], idx: int, model: str, expected_rid: str) -> None:
    rid = str(r.get("race_id", "")).strip()
    m = str(r.get("probability_source", ""))
    if rid != expected_rid or m != model:
        raise FailClosed(f"Stage2 immutable order drift at dev_index={idx}: got {rid}/{m}, expected {expected_rid}/{model}")
    if r.get("result_fields_included") is not False or r.get("settlement_fields_included") is not False or r.get("realized_roi_computed") is not False:
        raise FailClosed(f"Stage2 outcome firewall drift at {rid}/{m}")

def iter_stage2_pairs(path: Path, by_idx: dict[int, str]) -> Iterator[tuple[int, dict[str, Any], dict[str, Any]]]:
    """Stream exact pairs in the frozen Stage1->Stage2 order, holding one race at a time."""
    row_count = 0
    idx = 1
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            if not line.strip():
                continue
            if idx > 2000:
                raise FailClosed("Stage2 has more than 4000 nonblank rows")
            r = json.loads(line)
            model_pos = row_count % 2
            model = MODELS[model_pos]
            _validate_stage2_row(r, idx, model, by_idx[idx])
            row_count += 1
            if model_pos == 0:
                pending = r
            else:
                if pending is None:
                    raise FailClosed("Stage2 pairing state lost")
                yield idx, pending, r
                pending = None
                idx += 1
    if pending is not None or row_count != EXPECTED_STAGE2_ROWS or idx != 2001:
        raise FailClosed(f"Stage2 cardinality/pairing failure rows={row_count} next_index={idx}")
...
def allocate_stakes(selected: list[tuple[str, str, dict[str, float]]], policy: str, bankroll: int) -> list[tuple[str, str, dict[str, float], int]]:
    ...
    if policy == "RACE2PCT_EQUAL":
        ...
        return [(m, k, v, int((base + (1 if i < rem else 0)) * 100)) for i, (m, k, v) in enumerate(kept)]
    if policy in ("FK10_R2", "FK25_R3"):
        ...
        for _, _, v in ranked:
            ...
            kelly = 0.0 if o <= 1.0 else max(0.0, (o * p - 1.0) / (o - 1.0))
            raw.append(mult * kelly)
        raw_sum = sum(x for x in raw if x > 0)
        scale = race_cap / raw_sum if raw_sum > race_cap and raw_sum > 0 else 1.0
        out = []
...
class SegmentState:
    ...
    def settle_race(self, allocations: list[tuple[str, str, dict[str, float], int]], settlement: dict[str, Any]) -> dict[str, Any]:
        ...
        self.total_stake += int(stake); self.total_return += int(ret)
        if stake > 0: self.bet_races += 1
        ...
        return {
            "bankroll_before": before, "stake": stake, "return": ret,
            "bankroll_after": self.bankroll, "bet_ticket_count": len(allocations),
    ...
    def metrics(self) -> dict[str, Any]:
        ...
        return {
            "realized_roi": roi, "total_stake": self.total_stake, "total_return": self.total_return,
            "bet_race_count": self.bet_races, "hit_ticket_count": self.hit_tickets,
            "ending_bankroll": self.bankroll, "maximum_drawdown": self.max_drawdown,
            "minimum_bankroll": self.min_bankroll, "negative_bankroll": self.min_bankroll < 0,
        }
...
def metrics_rank_key(item: tuple[str, dict[str, Any]]):
    cid, m = item
    roi = m["realized_roi"] if m["realized_roi"] is not None else float("-inf")
    return (-float(roi), -int(m["ending_bankroll"]), float(m["maximum_drawdown"]), -int(m["bet_race_count"]), cid)

def selections_for_needed_bases(mod, rid: str, A: dict, B: dict, pred: dict, needed_bases: set[tuple[str, str, str]]):
    pmap = {x[0]: x for x in mod.PROFILES}
    gmap = {x[0]: x[1] for x in mod.GATES}
    out: dict[tuple[str, str, str], list[tuple[str, str, dict[str, float]]]] = {}
    pg_needed = sorted({(p,g) for p,g,_ in needed_bases})
    for pid, gid in pg_needed:
        _, elig = mod.consensus_eligible(A, B, pmap[pid], gmap[gid])
        for tpl in sorted(t for p,g,t in needed_bases if p == pid and g == gid):
            out[(pid,gid,tpl)] = global_rank(mod.select_template(rid, A, elig, pred, tpl))
    return out

def apply_race_to_configs(mod, rid: str, A: dict, B: dict, pred: dict,
                          settlement: dict[str, Any], states: dict[str, SegmentState],
                          cids: list[str], ledger_for: str | None = None) -> dict[str, Any] | None:
    bases = {parse_config(cid)[:3] for cid in cids}
    sel_by_base = selections_for_needed_bases(mod, rid, A, B, pred, bases)
    ledger_row = None
    for cid in cids:
        p,g,t,pol = parse_config(cid)
        st = states[cid]
        alloc = allocate_stakes(sel_by_base[(p,g,t)], pol, st.bankroll)
        race = st.settle_race(alloc, settlement)
        if cid == ledger_for:
            ledger_row = race
    return ledger_row
...
def _a_shortlist(A_states: dict[str, SegmentState]):
    A_metrics = {cid: st.metrics() for cid, st in A_states.items()}
    eligible = [(cid,m) for cid,m in A_metrics.items()
                if m["bet_race_count"] >= 100 and m["total_stake"] > 0 and
                   not m["negative_bankroll"] and m["maximum_drawdown"] <= 0.35]
    eligible.sort(key=metrics_rank_key)
    return eligible, eligible[:10]

def _b_pass(B_states: dict[str, SegmentState]):
    B_metrics = {cid: st.metrics() for cid, st in B_states.items()}
    passed = [(cid,m) for cid,m in B_metrics.items()
              if m["realized_roi"] is not None and m["realized_roi"] > 0 and
                 m["bet_race_count"] >= 50 and m["maximum_drawdown"] <= 0.35 and
                 not m["negative_bankroll"]]
    passed.sort(key=metrics_rank_key)
    return passed
...
def ab_phase(a, mod, by_idx, pred):
    ...
    if freeze_path.is_file() and ab_receipt.is_file():
    ...
    A_sett = load_settlement_segment(settlement_dir / "DEV2000_SETTLEMENT_A_v1.jsonl", "A", set(range(1,1001)), by_idx)
    B_sett = load_settlement_segment(settlement_dir / "DEV2000_SETTLEMENT_B_v1.jsonl", "B", set(range(1001,1501)), by_idx)

    all_cids = all_config_ids()
    if len(all_cids) != 784 or len(set(all_cids)) != 784:
        raise FailClosed("frozen configuration cardinality != 784")
    A_states = {cid: SegmentState() for cid in all_cids}
    top10 = None; eligible = None; B_states = None

    for idx, A, B in iter_stage2_pairs(Path(a.stage2_jsonl), by_idx):
        rid = by_idx[idx]
        if idx <= 1000:
            apply_race_to_configs(mod, rid, A, B, pred, A_sett[rid], A_states, all_cids)
            if idx == 1000:
                eligible, top10 = _a_shortlist(A_states)
                if not eligible:
                    halt = {"record":"STAGE7_AB_SELECTION_RECEIPT_v2","status":"NO_A_ELIGIBLE_CONFIGURATION","a_eligible_count":0,"segment_c_opened":False,"scientific_segment_c_scoring_count":0,"ECON_HOLDOUT1000":"SEALED"}
                    atomic_json(ab_receipt, halt)
                    raise FailClosed("NO_A_ELIGIBLE_CONFIGURATION; Segment C remains untouched")
                B_states = {cid: SegmentState() for cid,_ in top10}
        elif idx <= 1500:
            if top10 is None or B_states is None:
                raise FailClosed("A shortlist state missing before B")
            ids = [cid for cid,_ in top10]
            apply_race_to_configs(mod, rid, A, B, pred, B_sett[rid], B_states, ids)
            if idx == 1500:
                break

    if eligible is None or top10 is None or B_states is None:
        raise FailClosed("AB stream ended before frozen split completed")
    bpass = _b_pass(B_states)
    if not bpass:
        halt = {"record":"STAGE7_AB_SELECTION_RECEIPT_v2","status":"NO_B_VALIDATED_CONFIGURATION","A_TOP10":[{"configuration_id":cid, **m} for cid,m in top10],"b_pass_count":0,"segment_c_opened":False,"scientific_segment_c_scoring_count":0,"ECON_HOLDOUT1000":"SEALED"}
        atomic_json(ab_receipt, halt)
...
def _bootstrap_batch(batch_id: int, idx: Any, stake: Any, ret: Any):
    import numpy as np
    ss = stake[idx].sum(axis=1); rr = ret[idx].sum(axis=1)
    out = np.full(ss.shape, np.nan, dtype=np.float64); mask = ss > 0
...
def bootstrap_from_ledger(ledger: list[dict[str, Any]], log_path: Path) -> dict[str, Any]:
    ...
    indices = rng.integers(0, 500, size=(BOOTSTRAP_REPS,500), dtype=np.int32)
    batch_size = 250
    batches = [(i,indices[s:min(s+batch_size,BOOTSTRAP_REPS)]) for i,s in enumerate(range(0,BOOTSTRAP_REPS,batch_size))]
    results = {}
    ...
    with log_path.open("a", encoding="utf-8") as log:
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            futs = {ex.submit(_bootstrap_batch,bid,ii,stake,ret):bid for bid,ii in batches}
            for fut in as_completed(futs):
                bid, vals = fut.result(); results[bid] = vals
                log.write(f"BOOTSTRAP_BATCH_PASS {bid+1}/{len(batches)}\n"); log.flush()
        ordered = np.concatenate([results[i] for i in sorted(results)]); valid = ordered[np.isfinite(ordered)]
...
def validate_c_ledger(ledger: list[dict[str,Any]], cid: str) -> dict[str,Any]:
    if len(ledger) != 500:
        raise FailClosed("C ledger must contain exactly 500 rows")
    if [int(r["dev_index"]) for r in ledger] != list(range(1501,2001)):
        raise FailClosed("C ledger index sequence drift")
    total_stake=total_return=bet_races=hit_tickets=0
    peak=START_BANKROLL; max_dd=0.0; min_bank=START_BANKROLL; expected=START_BANKROLL
    for r in ledger:
        if r.get("configuration_id") != cid:
            raise FailClosed("C ledger config binding drift")
        before=int(r["bankroll_before"]); after=int(r["bankroll_after"]); stake=int(r["stake"]); ret=int(r["return"])
        if before != expected or after != before-stake+ret or after < 0:
            raise FailClosed("C ledger bankroll/accounting continuity failure")
        total_stake += stake; total_return += ret
        if stake > 0: bet_races += 1
        hit_tickets += int(r["hit_ticket_count"])
        peak=max(peak,after); max_dd=max(max_dd,(peak-after)/peak if peak>0 else 0.0); min_bank=min(min_bank,after); expected=after
    roi=total_return/total_stake-1.0 if total_stake>0 else None
    return {"realized_roi":roi,"total_stake":total_stake,"total_return":total_return,"bet_race_count":bet_races,"hit_ticket_count":hit_tickets,"ending_bankroll":expected,"maximum_drawdown":max_dd,"minimum_bankroll":min_bank,"negative_bankroll":min_bank<0}

def c_phase(a, mod, by_idx, pred):
    out=Path(a.out_dir)
    freeze_path=out/"FINAL_DEV2000_CONFIGURATION_FREEZE_v1.json"; final_receipt=out/"STAGE7_FINAL_OOS_RECEIPT_v1.json"
    c_receipt=out/"STAGE7_C_SINGLE_SHOT_RECEIPT_v1.json"; ledger_path=out/"SEGMENT_C_SINGLE_SHOT_LEDGER_v1.jsonl"
    lock_path=out/"SEGMENT_C_SCORE_LOCK_v1.json"; boot_log=out/"STAGE7_C_BOOTSTRAP_LOG_v1.txt"
    ...
    if freeze.get("status")!="FROZEN_BEFORE_SEGMENT_C_OPEN" or freeze.get("segment_c_opened") is not False or freeze.get("scientific_segment_c_scoring_count")!=0:
        raise FailClosed("final configuration freeze is not valid pre-C state")
    cid=str(freeze.get("final_configuration_id","")); parse_config(cid); freeze_sha=sha256_file(freeze_path)
    ...
    if lock_path.is_file():
        ...
        technical_resume=True
    else:
        atomic_json(lock_path,{"record":"SEGMENT_C_SCORE_LOCK_v1","status":"LOCKED_SINGLE_FROZEN_TRIAL","final_configuration_id":cid,"final_freeze_sha256":freeze_sha,"stage2_sha256":EXPECTED_STAGE2_SHA256,"stage456_git_blob":EXPECTED_STAGE456_BLOB,"stage7_prereg_blob":EXPECTED_STAGE7_PREREG_BLOB,"execution_conventions_blob":EXPECTED_EXEC_CONVENTIONS_BLOB,"bootstrap_reps":BOOTSTRAP_REPS,"bootstrap_seed":BOOTSTRAP_SEED,"scientific_segment_c_scoring_count":1,"ECON_HOLDOUT1000":"SEALED"})
    settlement_dir=Path(a.settlement_dir); load_bulk_receipt(settlement_dir)
    if ledger_path.is_file():
        ledger=load_jsonl(ledger_path); c_metrics=validate_c_ledger(ledger,cid)
    else:
        C_sett=load_settlement_segment(settlement_dir/"DEV2000_SETTLEMENT_C_UNTOUCHED_v1.jsonl","C",set(range(1501,2001)),by_idx)
        states={cid:SegmentState()}; ledger=[]
        for idx,A,B in iter_stage2_pairs(Path(a.stage2_jsonl),by_idx):
            if idx < 1501: continue
            rid=by_idx[idx]; race=apply_race_to_configs(mod,rid,A,B,pred,C_sett[rid],states,[cid],ledger_for=cid)
            if race is None: raise FailClosed("C race ledger output missing")
            ledger.append({"dev_index":idx,"race_id":rid,"configuration_id":cid,**race})
        c_metrics=validate_c_ledger(ledger,cid); atomic_jsonl(ledger_path,ledger)
    boot=bootstrap_from_ledger(ledger,boot_log); roi=c_metrics["realized_roi"]; bet=int(c_metrics["bet_race_count"]); dd=float(c_metrics["maximum_drawdown"]); lower=float(boot["roi_p2_5"])
    if roi is not None and roi>0 and bet>=50 and dd<=0.25 and lower>0: verdict="OOS_STRONG_PASS"
    elif roi is not None and roi>0 and bet>=50 and dd<=0.35 and lower<=0: verdict="OOS_POSITIVE_BUT_UNCERTAIN"
    else: verdict="OOS_FAIL"
    c_rec={"record":"STAGE7_C_SINGLE_SHOT_RECEIPT_v2","status":"PASS_SINGLE_SHOT_COMPLETE","final_configuration_id":cid,"final_freeze_sha256":freeze_sha,"technical_resume_same_frozen_trial":technical_resume,"ledger_sha256":sha256_file(ledger_path),"c_metrics":c_metrics,"bootstrap":boot,"oos_verdict":verdict,"scientific_segment_c_scoring_count":1,"ECON_HOLDOUT1000":"SEALED"}
    ...
    atomic_json(final_receipt,final); print(json.dumps(final,ensure_ascii=False,indent=2)); return 0
...
def self_test() -> int:
    ...
    print(json.dumps({"status":"SELF_TEST_PASS","configuration_count":784,"implementation":"streaming_v2"},indent=2)); return 0
