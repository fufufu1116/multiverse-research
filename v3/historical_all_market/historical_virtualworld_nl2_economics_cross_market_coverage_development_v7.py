...
import historical_virtualworld_nl2_economics_diversified_wide_portfolio_development_v6 as v6

v5 = v6.v5
engine = v6.engine
base = v6.base
ROOT = Path('v3/historical_all_market/research_candidates')
RULE = ROOT / 'KEIRIN_NL2_ECONOMICS_CROSS_MARKET_COVERAGE_DEVELOPMENT_PREREG_20260913_v7.json'
LEDGER = Path('v3/historical_all_market/continuity/KEIRIN_EXPERIMENT_LEDGER_v3.jsonl')
OUT = ROOT / 'nl2_economics_cross_market_coverage_development_v7'
OUT.mkdir(parents=True, exist_ok=True)

EXPECTED_RULE_SHA256 = 'ec679fce7b609f4bf8e7b83ab3e24103f8940a2dfc8b9abdc5e3552553cf6f94'
EXPECTED_NL2_SHA = v6.EXPECTED_NL2_SHA
EXPECTED_B1A_BLOB = v6.EXPECTED_B1A_BLOB
EXPECTED_POOL = v6.EXPECTED_POOL
EXPECTED_FOLDS = v6.EXPECTED_FOLDS
EXPECTED_V6_RUN = 34741203167
EXPECTED_V6_ARTIFACT = 10312882871
EXPECTED_ELIGIBILITY = v6.EXPECTED_ELIGIBILITY
...
def load_rule() -> dict:
    ...
    if hashlib.sha256(raw).hexdigest() != EXPECTED_RULE_SHA256:
        raise RuntimeError('v7_prereg_sha256_mismatch')
    return json.loads(raw.decode('utf-8'))


def load_v6_record() -> dict:
    found = []
    for line in LEDGER.read_text(encoding='utf-8').splitlines():
        if line.strip():
            row = json.loads(line)
            if row.get('experiment_id') == 'EXP-NL2-ECONOMICS-DIVERSIFIED-WIDE-PORTFOLIO-DEVELOPMENT-V6':
                found.append(row)
    if len(found) != 1 or found[0].get('status') != 'COMPLETE_NO_DEVELOPMENT_ELIGIBLE_POLICY':
        raise RuntimeError('v7_v6_ledger_binding_invalid')
    ev = found[0].get('evidence', {})
    if int(ev.get('run_id', -1)) != EXPECTED_V6_RUN or int(ev.get('artifact_id', -1)) != EXPECTED_V6_ARTIFACT:
        raise RuntimeError('v7_v6_evidence_binding_invalid')
    return found[0]


def validate_rule(rule: dict) -> None:
    if rule.get('record') != 'KEIRIN_NL2_ECONOMICS_CROSS_MARKET_COVERAGE_DEVELOPMENT_PREREG_20260913_v7':
        raise RuntimeError('v7_record_invalid')
    if rule.get('status') != 'PREREGISTERED_BEFORE_V7_EXPOSED_DEVELOPMENT_SCORING':
        raise RuntimeError('v7_status_invalid')
    if rule.get('lineage') != 'NL2_ECONOMICS_V7_CROSS_MARKET_COVERAGE':
        raise RuntimeError('v7_lineage_invalid')
    if rule.get('evidence_class') != 'EXPOSED_DEVELOPMENT_ONLY_NO_FRESH_VALIDATION_ACCESS':
        raise RuntimeError('v7_evidence_class_invalid')
    ...
    if expected_count != 72 or int(fam['configuration_count']) != 72:
        raise RuntimeError('v7_config_count_invalid')
    if rule['development_eligibility'] != EXPECTED_ELIGIBILITY:
        raise RuntimeError('v7_eligibility_invalid')


def config_space(rule: dict) -> list[dict]:
    fam = rule['purchase_policy_family']
    out = []
    for bundle in fam['market_bundles']:
        bname = '+'.join(str(x).upper() for x in bundle)
    ...
    if len(out) != 72 or len({x['configuration_id'] for x in out}) != 72:
        raise RuntimeError('v7_config_space_generation_invalid')
    return out


def cross_market_choices(race: dict, price: dict, rule: dict, configs: list[dict]) -> dict[str, list[dict]]:
    active = [int(x) for x in price['active_car_numbers']]
    pre_cars = sorted(int(x['car_no']) for x in race['inputs'])
    if active != pre_cars:
        raise RuntimeError('v7_active_car_mismatch')
    ...
    if set(pn) != set(active) or set(pb) != set(active):
        raise RuntimeError('v7_prediction_active_car_set_mismatch')

    sold = set(price['sold_markets'])
    all_needed = sorted({m for c in configs for m in c['markets'] if m in sold})
    keys = {m: list(price['closing_price_catalogs'][m].keys()) for m in all_needed}
    probs_n, _ = base.pl.build_market_probs(active, pn, all_needed, keys) if all_needed else ({}, None)
    probs_b, _ = base.pl.build_market_probs(active, pb, all_needed, keys) if all_needed else ({}, None)

    metrics: dict[str, list[dict]] = {}
    for m in all_needed:
        cat = price['closing_price_catalogs'][m]
        odds = {k: engine.primary_odds(m, v) for k, v in cat.items()}
        if any((not math.isfinite(o) or o <= 0) for o in odds.values()):
            raise RuntimeError(f'v7_invalid_odds_{m}')
        inv_sum = sum(1.0 / o for o in odds.values())
        if not math.isfinite(inv_sum) or inv_sum <= 0:
            raise RuntimeError(f'v7_invalid_inv_sum_{m}')
        target_sum = 3.0 if m == 'wide' else 1.0
        mm = []
        for k in sorted(cat):
            o = odds[k]
            p_n = float(probs_n[m][k])
            p_b = float(probs_b[m][k])
            p_lo = min(p_n, p_b)
            p_hi = max(p_n, p_b)
            q = (1.0 / o) / inv_sum
            mm.append({
                ...
                'conservative_shape_edge_ratio': min((p_n / target_sum) / q, (p_b / target_sum) / q),
            })
        metrics[m] = mm

    out = {}
    for c in configs:
        if any(m not in sold for m in c['markets']):
            out[c['configuration_id']] = []
            continue
        selected = []
        for m in c['markets']:
        ...
        out[c['configuration_id']] = selected
    return out
...
def main() -> None:
    ...
    if nl2['model_core_sha256'] != EXPECTED_NL2_SHA:
        raise RuntimeError('v7_loaded_nl2_sha_mismatch')
    base.ticket_choices = cross_market_choices

    dev, rejected = base.collect_pre_price(rule['development_pool']['meetings'], rule, configs, 'V7_EXPOSED_DEVELOPMENT', nl1, nl2)
    counts = dict(sorted(Counter(r['meeting_id'] for r in dev).items()))
    dump('01_DEVELOPMENT_PRE_PRICE_LOCK.json', {
        'status': 'FROZEN_BEFORE_V7_EXPOSED_SETTLEMENT_SCORING',
    ...
    dump('02_DEVELOPMENT_SCORE.json', {
        'record': 'KEIRIN_NL2_ECONOMICS_V7_DEVELOPMENT_SCORE',
    ...
    if winner is None:
        freeze = {
            'record':'KEIRIN_NL2_ECONOMICS_V7_POLICY_FREEZE',
        ...
        freeze = {
            'record':'KEIRIN_NL2_ECONOMICS_V7_POLICY_FREEZE',
            'status':'FROZEN_AFTER_EXPOSED_V7_DEVELOPMENT_BEFORE_ANY_FRESH_VALIDATION_PREREG',
    ...
    print('ECONOMICS_V7_DEVELOPMENT_SUMMARY=' + json.dumps({
