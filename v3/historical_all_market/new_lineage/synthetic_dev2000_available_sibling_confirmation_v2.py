...
ROOT = HERE.parent
PREREG = ROOT / "research_candidates" / "KEIRIN_SYNTHETIC_DEV2000_AVAILABLE_SIBLING_CONFIRMATION_PREREG_20260914_v2.json"
OUT = ROOT / "research_candidates" / "synthetic_dev2000_available_sibling_confirmation_v2"
EPS = 1e-15
...
def forecast(race: twin.Race, cfg: dict, variant: str) -> dict:
    ...
    for first in cars:
        ...
        for candidate in cars:
            ...
            p2[(first, candidate)] = base[candidate] + float(rel["p2_same_line"]) * float(same) + float(rel["p2_immediate_follower"]) * float(follower)
    for first in cars:
        rf = riders[first]
        for second in cars:
            if second == first:
            ...
            for candidate in cars:
                ...
                chain = use_group and use_pos and rf.line_id == rs.line_id == rc.line_id and rf.line_position < rs.line_position < rc.line_position
                p3[(first, second, candidate)] = base[candidate] + float(rel["p3_same_as_first"]) * float(same_f) + float(rel["p3_same_as_second"]) * float(same_s) + float(rel["p3_ordered_line_chain"]) * float(chain)
...
def main() -> int:
    ...
    if rule.get("status") != "PREREGISTERED_BEFORE_SYNTHETIC_CONFIRMATION_SCORING":
    ...
    batches = []
    for batch in rule["confirmation_batches"]:
        seed = int(batch["seed"])
    ...
    mapping = {"line_group_id":"ABLATE_LINE_GROUP_ID","line_position":"ABLATE_LINE_POSITION","line_size":"ABLATE_LINE_SIZE","bank_length_m":"ABLATE_BANK_LENGTH_M","wind_speed_mps":"ABLATE_WIND_SPEED_MPS"}
    dependence = {}
    for field, model in mapping.items():
        dependence[field] = {}
        for world in worlds:
            ratios = [b["worlds"][world]["ratio_to_N2_FULL"][model] for b in batches]
            dependence[field][world] = {"ratios":ratios,"material_all_three_seeds":all(x >= 1.10 for x in ratios)}
    ...
    confirmed = all(all(x <= 1.25 for x in sibling_ratios[w]) for w in worlds)
    classification = "S0_BROAD_SYNTHETIC_VIABILITY_CONFIRMED" if confirmed else "S0_BROAD_SYNTHETIC_VIABILITY_NOT_CONFIRMED_NO_TUNING"
    result = {"record":"KEIRIN_SYNTHETIC_DEV2000_AVAILABLE_SIBLING_CONFIRMATION_RESULT_v2","status":"COMPLETE_SYNTHETIC_CONFIRMATION","evidence_class":"SYNTHETIC_ENGINEERING_FALSIFICATION_ONLY","batches":batches,"field_dependence":dependence,"sibling_ratios_to_N2_FULL":sibling_ratios,"decision":{"confirmed":confirmed,"classification":classification},"frozen_N2_unchanged":True,"model_promotion":False,"real_world_claim":False,"protected_boundaries":p}
    dump("00_GOVERNANCE_AUDIT.json", {"status":"PASS_SYNTHETIC_ONLY","real_historical_input":False,"real_PRE_row_values":False,"RESULT_PAYOUT":"NOT_ACCESSED","ECON_HOLDOUT1000":"SEALED_NOT_ACCESSED","runtime":"OFF","automatic_betting":False,"frozen_N2_unchanged":True})
